...
Parameters $c_{1}$ and $c_{2}$ are integer variables which determine the shape of the specific cubic equation of state. In addition, the variable $a$ is considered to be temperature dependant in all modified cubic equations of state; the functional form of which is determined by fitting experimental vapour pressure data. Modern cubic equations of state which find widespread practical application are summarised in table \ref{CubicEOSTable}~\cite{ThermophysicalProperties, GasLiquidProperties, EOSModification}.\\
